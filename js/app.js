...
new slim_select__WEBPACK_IMPORTED_MODULE_1__["default"]({
  select: '#selectElementAdult',
  settings: {
    placeholderText: 'Кол-во взрослых',
    showSearch: false
  }
});
new slim_select__WEBPACK_IMPORTED_MODULE_1__["default"]({
  select: '#selectElementKids',
  settings: {
    placeholderText: 'Кол-во детей',
    showSearch: false
  }
});
...
// Slider ===================================================================
//First Screen
new swiper__WEBPACK_IMPORTED_MODULE_2__["default"]('.first-screen__media-slider-body', {
  direction: 'horizontal',
  spaceBetween: 15,
  slidesPerView: 1.3,
  breakpoints: {
...
new swiper__WEBPACK_IMPORTED_MODULE_2__["default"]('.card__media-body', {
  modules: [swiper__WEBPACK_IMPORTED_MODULE_2__.Navigation, swiper__WEBPACK_IMPORTED_MODULE_2__.Pagination],
  direction: 'horizontal',
  spaceBetween: 15,
  slidesPerView: 1,
  navigation: {
    prevEl: ".card__media-control-navigation.navigation--prev",
    nextEl: ".card__media-control-navigation.navigation--next"
  },
  pagination: {
    el: ".card__media-control-pagination",
    type: 'fraction'
  },
  breakpoints: {
    768: {
...
//Input ===================================================================
const inputBlocks = document.querySelectorAll('.input--type-date');
for (let inputBlock of inputBlocks) {
  const input = inputBlock.querySelector(".input--type-date__input");
...
if (isMobile.any()) {
  ...
  let menuArrows = document.querySelectorAll('.item-select__button');
  if (menuArrows.length > 0) {
    for (let index = 0; index < menuArrows.length; index++) {
      const menuArrow = menuArrows[index];
      const parentMenuArrow = menuArrow.parentElement;
      const closeButton = parentMenuArrow.querySelector('.item-select__close');
      menuArrow.addEventListener("click", function (e) {
        if (parentMenuArrow.classList.contains('_active')) {
          parentMenuArrow.classList.remove('_active');
        } else {
          for (let index = 0; index < menuArrows.length; index++) {
            const menuArrow = menuArrows[index];
            menuArrow.parentElement.classList.remove('_active');
          }
          parentMenuArrow.classList.add('_active');
        }
      });
...
const itemSelectedButtons = document.querySelectorAll('.item-select__button');
if (itemSelectedButtons.length > 0) {
  for (let itemSelectedButton of itemSelectedButtons) {
    const parentBtn = itemSelectedButton.parentElement;
    const items = parentBtn.querySelectorAll('.filter-radio');
    for (let item of items) {
      const itemContent = item.querySelector('.filter-radio__content');
      item.addEventListener('click', function () {
        itemSelectedButton.innerHTML = itemContent.innerHTML;
      });
      document.addEventListener('DOMContentLoaded', function () {
        if (item.querySelector('.filter-radio__input').checked) {
          itemSelectedButton.innerHTML = itemContent.innerHTML;
        }
      });
    }
